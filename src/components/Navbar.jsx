import React from 'react';
import logoimage from "../assets/images/skillslogo.png";

function Navbar() {
  return (
    // <header className="fixed left-[50%] transform-[translateX(-50%)] top-0 z-50 border-b border-black px-4 w-[100%]" style={{ backgroundColor: 'rgba(0,0,0,0)' }}>

    //   <div className="container mx-auto bg-transparent">
    //     <div className="flex justify-between items-center h-16 md:grid md:grid-cols-7 bg-transparent">
    //       {/* Logo */}
    //       <div className="flex justify-center items-center">
    //         <img src={logoimage} alt="skillspp" className="h-8 w-8" />
    //       </div>

    //       {/* Hidden on Mobile */}
    //       <div className="hidden md:grid md:col-span-5 grid-cols-4 h-full text-black">
    //         <div className="flex items-center justify-center border-x border-black h-full">
    //           <span className="text-base font-[Unbounded] font-medium">Loading...</span>
    //         </div>
    //         <div className="flex items-center justify-center border-r border-black h-full">
    //           <span className="text-base font-[Unbounded] font-medium">Processing...</span>
    //         </div>
    //         <div className="flex items-center justify-center border-r border-black h-full">
    //           <span className="text-base font-[Unbounded] font-medium">Rebooting...</span>
    //         </div>
    //         <div className="flex items-center justify-center border-r border-black h-full">
    //           <span className="text-base font-[Unbounded] font-medium">Coming Soon</span>
    //         </div>
    //       </div>

    //       {/* Button */}
    //       <div className="flex justify-center items-center w-max">
    //         <button className="px-4 py-2 ml-4 text-xs md:text-sm font-semibold bg-[#283593] text-white w-fit transition-all shadow-[4px_4px_0px_black] 
    //                hover:shadow-none hover:translate-x-[4px] hover:translate-y-[4px] uppercase tracking-wide font-[Unbounded]">
    //           TO BE HERE SOON
    //         </button>
    //       </div>
    //     </div>
    //   </div>
    // </header>
    <></>
  );
}

export default Navbar;
